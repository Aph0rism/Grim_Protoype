namespace Settings
{
    /// <summary>
    /// Regroupe tous les noms des tags du projet
    /// </summary>
    public class Tags
    {
        public string PLAYER_TAG = "Player";
        public string MAIN_CAMERA_TAG = "MainCamera";
        public string FLOOR_TAG = "Floor";
        public string WALL_TAG = "Wall";
        public string DANGER_TAG = "Danger";
        public string INTERACT_TAG = "InteractIndicator";
        public string INTERACTABLE_TAG = "Interactable";
        public string GAME_INPUT_TAG = "GameInput";
        public string PLAYER_SPAWN_TAG = "PlayerSpawn";
        public string PLAYER_CANVAS_TAG = "PlayerCanvas";
        public string EVENT_MANAGER_TAG = "EventManager";
        public string GAME_SYSTEM_TAG = "GameSystem";
        public string STORAGE_MENU_TAG = "StorageMenu";
    }
}
